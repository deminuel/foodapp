  ...
  <nav className="flex justify-between p-4 bg-[#EEBE1F]">
    ...
    <div className="relative w-full">
      <input
        ...
        className="hidden lg:flex lg:flex-[60%] w-full border-none outline-blue-200 bg-white rounded-lg h-[42px] indent-24"
        type="text"
        placeholder="Order Your Package"
      />
      <FaMagnifyingGlass className="absolute top-4 left-4 hidden lg:flex text-[14px] text-black/45" />
    </div>

    {/* === Right: Cart + Bell (Desktop) === */}
    <div className=" lg:flex lg:flex-[20%] justify-end gap-3 relative flex">
      <BiCart
        className="text-2xl hover:cursor-pointer"
        onClick={() => setShowCart(!showCart)}
      />
      <span className="absolute -top-3 right-8 bg-white rounded-full size-5 text-center">
        {totalItems}
      </span>

      {/* === Cart Dropdown === */}
      {showCart && (
        <div className="absolute right-0 mt-6 w-60 bg-white border rounded-lg shadow-lg p-3 z-50">
    ...
    <div className="lg:hidden flex flex-col pb-5 relative">
      ...
      <div className="relative w-full">
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="flex w-fit md:w-[80%] border-none outline-blue-200 bg-white rounded-lg h-[42px] indent-7 m-auto"
          type="text"
          placeholder="Order Your Package"
        />
      ...
      {/* <div className=" flex flex-col gap-3 mt-4">
        <div className='relative inline-block'>
          ...
          onClick={() => setShowCart(!showCart)}
        />
        <span className="absolute -top-2 md:left-96 right-[170px] bg-white rounded-full size-5 text-center">
          {totalItems}
        </span>
        </div>
  ...
  {/* Carousel */}
    ...
    <article className='bg-white flex flex-col'>
     ...
     <div className='flex flex-col '>
      ...
      {/* <MobileNav  addToCart={addToCart} cart={cart} removeFromCart={removeFromCart}/> */}
      </>
  );
}
